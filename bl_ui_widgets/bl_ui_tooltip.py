...
class BL_UI_Tooltip(BL_UI_Patch):
    ...
    def __init__(self):
        # These arguments have no use in the case of a tooltip object
        super().__init__(0, 0, 0, 0)

        self._text_color = None                                # Tooltip text color
        # Tooltip shortcut color medium gray (seems to be fixed)
        self._shortcut_color = (185/255, 185/255, 185/255, 1)
        # Tooltip python cmd color dark gray (seems to be fixed)
        self._python_color = (122/255, 122/255, 122/255, 1)

        # Note: '_style' value will always be ignored if the bg_color value is overriden after object initialization.

        # Tooltip background color styles are: {HEADER,PANEL,SUBPANEL,TOOLTIP,NONE}
        self._style = 'TOOLTIP'
        # Tooltip background color (defaults to 'TOOLTIP')
        self._bg_color = None
        # Tooltip outline color (defaults to 'TOOLTIP')
        self._outline_color = None
        # Tooltip corners roundness factor [0..1]
        ...
        self._rounded_corners = (1, 1, 1, 1)
        # Indicates whether a shadow must be drawn around the tooltip
        self._has_shadow = True

        self._text_size = None                  # Tooltip text line 1 size
        # Tooltip text kerning (True/False)
        self._text_kerning = None
        self._text_shadow_size = None           # Tooltip text shadow size
        # Tooltip text shadow offset x (positive goes right)
        self._text_shadow_offset_x = None
        # Tooltip text shadow offset y (negative goes down)
        self._text_shadow_offset_y = None
        # Tooltip text shadow color [0..1] = gray tone, from dark to clear
        self._text_shadow_color = None
        # Tooltip text shadow alpha value [0..1]
        self._text_shadow_alpha = None

        # Indicates that object generated by this class is a Tooltip type
        self._is_tooltip = True
        ...
        # Identifies the widget object to have tooltip displayed
        self.__tooltip_widget = None
        # Text size used on drawing the tooltip last time
        self.__tooltip_textsize = None
        # Text kerning used on drawing the tooltip last time
        self.__tooltip_textkern = None
        # Former setup for displaying python commands
        self.__tooltip_showpyth = None
        # Tooltip text lines to be prepared by internal function
        self.__tooltip_text_lines = []
        # Tooltip shortcut text lines to be prepared by internal function
        self.__tooltip_shortcut_lines = []
        # Tooltip python text lines to be prepared by internal function
        self.__tooltip_python_lines = []

        # Standard vertical spacing between tooltip text lines (units in pixels)
        self.__line_space = 6
        # Standard margin size for tooltip text arrangement (units in pixels)
        self.__text_margin = 12

        # Limit of 3 text lines for the main tooltip
        self.__max_lines_text = 24
        self.__max_lines_shortcut = 1           # Limit of 1 text line for the shortcut
        # Limit of 2 text lines for the python command
        self.__max_lines_python = 2
        # Limit of 450px for the tooltip box width (which is variable)
        self.__max_tooltip_width = 450
    ...
    def prepare_tooltip_data(self, widget):
        ...
        if self.__tooltip_widget is not widget:
            ...
            if self.__area_height != area_height or self.__over_scale != self.over_scale(1):
                ...
                # 3.00 issue: 'font_kerning_style' has become extinct
                if bpy.app.version >= (3, 0, 0):
                    text_kerning = True if self._text_kerning is None else self._text_kerning
                else:
                    text_kerning = (widget_style.font_kerning_style ==
                                    'FITTED') if self._text_kerning is None else self._text_kerning
    ...
    def get_tooltip_measurements(self):
        ...
        # This is to keep a regular pattern since letters differ in height
        text_normal = blf.dimensions(0, "W")[1]
        ...
        if widget.description != "":
            line_count = 0
            self.__tooltip_text_lines = self.text_wrap(
                widget.description, scaled_text_size, text_kerning, scaled_max_width, self.__max_lines_text)
        ...
        if widget.shortcut != "":
            line_count = 0
            self.__tooltip_shortcut_lines = self.text_wrap(
                widget.shortcut, scaled_text_size, text_kerning, scaled_max_width, self.__max_lines_shortcut)
        ...
        if widget.python_cmd != "" and display_python:
            line_count = 0
            self.__tooltip_python_lines = self.text_wrap(
                widget.python_cmd, scaled_text_size, text_kerning, scaled_max_width, self.__max_lines_python)
        ...
        if self.over_scale(tooltip_x) + self.ui_scale(total_width) > area_width - 50:
            tooltip_x = (area_width - self.ui_scale(total_width) -
                         50) / self.over_scale(1)
        ...
        if self.over_scale(tooltip_y) - self.ui_scale(total_height) < 12:
            tooltip_y = (self.over_scale(widget.y_screen) +
                         self.ui_scale(total_height) + 12) / self.over_scale(1)
    ...
    def draw_text(self):
        ...
        textpos_x = self.x + \
            (self.ui_scale(self.__text_margin) / over_scale) - 1
        textpos_y = self.y - \
            (self.ui_scale(self.__text_margin +
             round(text_normal + 0.499)) / over_scale) + 1
        ...
        if len(self.__tooltip_text_lines) > 0:
            ...
            for line in self.__tooltip_text_lines:
                ...
                line_count += 1
                # Need to unapply the over scale to compensate for posterior calculations
                textpos_y -= (round(text_height + 0.499) +
                              line_spacing) / over_scale
